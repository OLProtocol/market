...
noble_secp256k1.etc.hmacSha256Sync = function (key) {
  var _noble_secp256k1$etc;
  for (
  ...
  return hmac.hmac(
    sha256.sha256,
    key,
    (_noble_secp256k1$etc = noble_secp256k1.etc).concatBytes.apply(
      _noble_secp256k1$etc,
      msgs,
    ),
  );
};
...
export function signMessageOfDeterministicECDSA(ecpair, message) {
  ...
  var signature = noble_secp256k1.sign(
    Buffer.from(hash),
    ecpair.privateKey.toString('hex'),
    { lowS: true },
  );
  return toCompact(signature.recovery, signature.toCompactRawBytes(), true).toString('base64');
}
