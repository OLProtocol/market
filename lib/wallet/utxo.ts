...
export const getSuitableUtxos = ({
  ...
  // const inputs = converUtxosToInputs({
  //   utxos,
  //   address,
  // });
  const psbtNetwork =
  ...
  const outputAmount = outputs.reduce((acc, v) => acc + v.amount, 0);
  let total = 0;
  const fixedInputs: any[] = [];
  const networksInputs: any = [];
  for (let i = 0; i < utxos.length; i++) {
    const input = utxos[i];

    if (total > outputAmount) {
      networksInputs.push(input);
    } else {
      fixedInputs.push(input);
    }
    total += input.value;
  }
  console.log('getSuitableUtxos networksInputs', networksInputs);
  console.log('getSuitableUtxos networksInputs', fixedInputs);
  console.log(
    'getSuitableUtxos networksInputs',
    fixedInputs.reduce((acc, v) => acc + v.value, 0),
  );
  const inputsData = fixedInputs.map((v) => {
    return {
      txId: v.txid,
      vOut: v.vout,
      amount: v.value,
  ...
  const networkData = networksInputs.map((v) => {
    return {
      txId: v.txid,
      vOut: v.vout,
      amount: v.value,
  ...
  console.log('getSuitableUtxos utxoTx', utxos);
  let fee = estimateBtcFee(utxoTx, psbtNetwork);
  if (fee < 111) {
    for (let j = 0; j < networkData.length; j++) {
      utxoTx.inputs.push(networkData[j]);
      fee = estimateBtcFee(utxoTx, psbtNetwork);
      if (fee >= 111) {
        break;
      }
    }
  }
  console.log('getSuitableUtxos fee', fee);
  console.log('getSuitableUtxos utxoTx', utxoTx);
};
