...
import { addOrderTask, getLastOrderTaskByParameters, getSatsByAddress, getUtxoByValue } from "@/api";
...
export default function HuntSatTool() {
    ...
    const doSearchInGlobal = async () => {
        ...
        try {
            ...
            if (!utxo) {
                ...
                notification.error({
                    message: t('notification.search_sats_title'),
                    description: 'Search sats in global failed: There is not enough sats to pay.',
                });
                return;
            }
            ...
            const txid = 'a2f42b3860bf09f4ef59d550d682a0c3cac2f74adcf90c04ff8da6777c519d42';

            const parameters = JSON.stringify({
                sat_no: searchSatList.items[0].sat,
            });

            const resp = await addOrderTask({ address: currentAccount, fees: serviceFee, parameters, txid, type }); // 提交任务
            setLoading(false);
            if (resp.code === 200) {
    ...
    const getOrderTaskBySat = async () => {
        ...
        const resp = await getLastOrderTaskByParameters({ address: currentAccount, parameters, type: 'search_rarity_sats' });
        if (resp.code === 200) {
            task = resp.data;
        }
        return task;
            ...
            <Card>
                <CardBody>
                    ...
                    <div className="flex gap-2 pb-2 justify-end">
                        <Switch defaultSelected={selectGlobal} onValueChange={() => handleSwitch(!selectGlobal)}>
